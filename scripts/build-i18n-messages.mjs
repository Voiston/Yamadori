...
t('gdd_base_temp', 'T° base : {temp} °C', 'Base T°: {temp} °C', 'Basis-T°: {temp} °C', 'T° base: {temp} °C', 'T° base: {temp} °C');
...
t('yrs_timing_hint', 'Score de timing (climat / sol) — pas une note esthétique, ni un permis ou feu vert légal.', 'Harvest-timing score (climate / soil) — not an aesthetic grade, permit, or legal go-ahead.', 'Timing-Score (Klima / Boden) — keine Ästhetiknote, keine Genehmigung, kein rechtliches Grünlicht.', 'Punteggio di timing (clima / suolo) — non un voto estetico, né un permesso o via libera legale.', 'Puntuación de timing (clima / suelo) — no es nota estética, permiso ni luz verde legal.');
...
t('yrs_total', 'Total : {value}', 'Total: {value}', 'Gesamt: {value}', 'Totale: {value}', 'Total: {value}');
t('yrs_confidence_label', 'Confiance', 'Confidence', 'Konfidenz', 'Affidabilità', 'Confianza');
t('yrs_confidence_high', 'Élevée', 'High', 'Hoch', 'Alta', 'Alta');
t('yrs_confidence_medium', 'Moyenne', 'Medium', 'Mittel', 'Media', 'Media');
t('yrs_confidence_low', 'Faible', 'Low', 'Niedrig', 'Bassa', 'Baja');
t('yrs_confidence_hint', 'selon les données météo et terrain disponibles', 'based on available weather and field data', 'basierend auf verfügbaren Wetter- und Felddaten', 'in base ai dati meteo e di campo disponibili', 'según los datos meteorológicos y de campo disponibles');
t('yrs_confidence_cta_phenology', 'Ajoutez une observation de phénologie pour hausser la confiance.', 'Add a phenology observation to raise confidence.', 'Phänologie-Beobachtung hinzufügen, um die Konfidenz zu erhöhen.', 'Aggiungi un’osservazione fenologica per alzare la confidenza.', 'Añade una observación de fenología para subir la confianza.');
t('yrs_confidence_cta_species', 'Précisez une espèce reconnue pour affiner GDD et calendrier.', 'Specify a recognized species to refine GDD and calendar.', 'Bekannte Art angeben, um GDD und Kalender zu verfeinern.', 'Indica una specie riconosciuta per affinare GDD e calendario.', 'Indica una especie reconocida para afinar GDD y calendario.');
t('yrs_confidence_cta_hydric', 'Données hydriques limitées — la confiance reste prudente.', 'Limited hydric data — confidence stays cautious.', 'Begrenzte Hydrikdaten — Konfidenz bleibt vorsichtig.', 'Dati idrici limitati — confidenza prudente.', 'Datos hídricos limitados — confianza prudente.');
t('yrs_confidence_cta_gdd', 'GDD indisponible — vérifiez la connexion ou réessayez plus tard.', 'GDD unavailable — check connectivity or try again later.', 'GDD nicht verfügbar — Verbindung prüfen oder später erneut versuchen.', 'GDD non disponibile — controlla la connessione o riprova più tardi.', 'GDD no disponible — comprueba la conexión o inténtalo más tarde.');
t('yrs_localization_generic_hint', 'Timing basé sur des seuils généraux — vérifier le calendrier local.', 'Timing based on general thresholds — check the local calendar.', 'Timing auf allgemeinen Schwellen — lokales Kalenderfenster prüfen.', 'Timing basato su soglie generali — verificare il calendario locale.', 'Timing basado en umbrales generales — verificar el calendario local.');
...
t('yrs_climate_summary_harvest_calendar', '{label} — hors fenêtre calendaire de prélèvement pour cette espèce / région.', '{label} — outside the regional harvest calendar window for this species.', '{label} — außerhalb des regionalen Sammelfensters für diese Art.', '{label} — fuori dalla finestra del calendario di prelievo per questa specie / regione.', '{label} — fuera de la ventana del calendario de recolección para esta especie / región.');
t('yrs_climate_summary_low_confidence', '{label} — confiance faible — vérifier sur le terrain.', '{label} — low confidence — verify in the field.', '{label} — geringe Konfidenz — vor Ort prüfen.', '{label} — confidenza bassa — verificare sul terreno.', '{label} — confianza baja — verificar en el terreno.');
t('yrs_breakdown_harvest_calendar_outside', 'Hors fenêtre calendaire régionale', 'Outside regional harvest calendar', 'Außerhalb regionales Sammelfenster', 'Fuori calendario regionale di prelievo', 'Fuera del calendario regional de recolección');
t('yrs_chart_outside_calendar', 'Hors calendrier régional', 'Outside regional calendar', 'Außerhalb regionales Kalender', 'Fuori calendario regionale', 'Fuera del calendario regional');
t('gdd_season_start_jan', 'Cumul depuis le {date} (hémisphère nord)', 'Cumulative since {date} (northern hemisphere)', 'Kumuliert seit {date} (Nordhalbkugel)', 'Cumulato dal {date} (emisfero nord)', 'Acumulado desde {date} (hemisferio norte)');
t('gdd_season_start_july', 'Cumul depuis le {date} (hémisphère sud)', 'Cumulative since {date} (southern hemisphere)', 'Kumuliert seit {date} (Südhalbkugel)', 'Cumulato dal {date} (emisfero sud)', 'Acumulado desde {date} (hemisferio sur)');
t('climate_profile_label', 'Profil climatique : {profile}', 'Climate profile: {profile}', 'Klimaprofil: {profile}', 'Profilo climatico: {profile}', 'Perfil climático: {profile}');
...
t('compass_dial_north', 'N', 'N', 'N', 'N', 'N');
t('format_distance_m', '{n} m', '{n} m', '{n} m', '{n} m', '{n} m');
t('format_distance_km', '{n} km', '{n} km', '{n} km', '{n} km', '{n} km');
t('pro_badge_short', 'PRO', 'PRO', 'PRO', 'PRO', 'PRO');
t('map_source_parcel_data', 'données parcellaires', 'parcel data', 'Flurstücksdaten', 'dati particellari', 'datos de parcela');
t('map_source_protected_areas', 'zones protégées', 'protected areas', 'Schutzgebiete', 'aree protette', 'áreas protegidas');
...
t('photo_add', 'Ajouter une photo', 'Add a photo', 'Foto hinzufügen', 'Aggiungi una foto', 'Añadir una foto');
t('photo_remove', 'Supprimer la photo', 'Remove photo', 'Foto entfernen', 'Rimuovi foto', 'Eliminar foto');
t('photo_slot_of', 'Photo {n} sur {max}', 'Photo {n} of {max}', 'Foto {n} von {max}', 'Foto {n} di {max}', 'Foto {n} de {max}');
t('photo_hint_overview', 'Vue d’ensemble', 'Overview', 'Übersicht', 'Panoramica', 'Vista general');
t('photo_hint_nebari', 'Nebari', 'Nebari', 'Nebari', 'Nebari', 'Nebari');
t('photo_hint_detail', 'Détail', 'Detail', 'Detail', 'Dettaglio', 'Detalle');
t('photo_drag_reorder', 'Glisser pour réordonner', 'Drag to reorder', 'Zum Neuanordnen ziehen', 'Trascina per riordinare', 'Arrastra para reordenar');
t('photo_edit_visit', 'Modifier les photos', 'Edit photos', 'Fotos bearbeiten', 'Modifica foto', 'Editar fotos');
t('photo_save_visit', 'Enregistrer les photos', 'Save photos', 'Fotos speichern', 'Salva foto', 'Guardar fotos');
...
t('climate_frost_year', '{days} j · {year}', '{days} d · {year}', '{days} T · {year}', '{days} g · {year}', '{days} d · {year}');
t('climate_frost_average', 'Moyenne : {days} jours/an', 'Average: {days} days/year', 'Mittel: {days} Tage/Jahr', 'Media: {days} giorni/anno', 'Media: {days} días/año');
t('climate_precip_annual', '{n} mm/an', '{n} mm/year', '{n} mm/Jahr', '{n} mm/anno', '{n} mm/año');
...
t('cadastre_law_link', 'Lire le texte sur {source}', 'Read the text on {source}', 'Text auf {source} lesen', 'Leggi il testo su {source}', 'Leer el texto en {source}');
t(
...
t('veto_species_inpn_link', 'Rechercher sur {source}', 'Search on {source}', 'Auf {source} suchen', 'Cerca su {source}', 'Buscar en {source}');
...
t('settings_dark_hint', 'Fond noir pur pour économiser la batterie sur écran OLED.', 'Pure black background to save battery on OLED screens.', 'Reiner schwarzer Hintergrund zum Akkusparen auf OLED-Bildschirmen.', 'Sfondo nero puro per risparmiare batteria su schermi OLED.', 'Fondo negro puro para ahorrar batería en pantallas OLED.');
...
t('settings_backup_hint', 'Sauvegarde complète de vos données. Sans mot de passe, le fichier est lisible hors de Yamadori. Cochez « Protéger l\'archive » pour le chiffrer.', 'Full backup of your data. Without a password, the file is readable outside Yamadori. Check “Protect archive” to encrypt it.', 'Vollständige Sicherung Ihrer Daten. Ohne Passwort ist die Datei außerhalb von Yamadori lesbar. Aktivieren Sie „Archiv schützen“, um sie zu verschlüsseln.', 'Backup completo dei tuoi dati. Senza password il file è leggibile fuori da Yamadori. Attiva « Proteggi archivio » per cifrarlo.', 'Copia completa de tus datos. Sin contraseña, el archivo es legible fuera de Yamadori. Marca « Proteger archivo » para cifrarlo.');
...
t('privacy_pillar_local_body', 'Pas de compte, pas de serveur Yamadori. Vos repérages, photos, notes vocales et point de départ restent sur cet appareil. Par défaut, les données sont stockées sans chiffrement dans le stockage local ; vous pouvez activer le chiffrement local dans Réglages (optionnel, Android) — il protège arbres, parking, mot de passe d’export et réglages GPS, mais pas les caches météo ou carte.', 'No account, no Yamadori server. Your scouting entries, photos, voice notes and starting point stay on this device. By default, data is stored unencrypted in local storage; you can enable local encryption in Settings (optional, Android) — it protects trees, parking, the export password and GPS settings, but not weather or map caches.', 'Kein Konto, kein Yamadori-Server. Ihre Sichtungen, Fotos, Sprachnotizen und Startpunkt bleiben auf diesem Gerät. Standardmäßig werden Daten unverschlüsselt im lokalen Speicher abgelegt; Sie können die lokale Verschlüsselung unter Einstellungen aktivieren (optional, Android) — sie schützt Bäume, Parkplatz, Exportpasswort und GPS-Einstellungen, aber nicht Wetter- oder Karten-Caches.', 'Nessun account, nessun server Yamadori. I tuoi rilevamenti, foto, note vocali e punto di partenza restano su questo dispositivo. Di default i dati sono salvati senza cifratura nello storage locale; puoi attivare la cifratura locale in Impostazioni (opzionale, Android) — protegge alberi, parcheggio, password di export e impostazioni GPS, ma non le cache meteo o mappa.', 'Sin cuenta, sin servidor Yamadori. Tus registros, fotos, notas de voz y punto de partida permanecen en este dispositivo. Por defecto los datos se guardan sin cifrado en el almacenamiento local; puedes activar el cifrado local en Ajustes (opcional, Android) — protege árboles, aparcamiento, contraseña de exportación y ajustes GPS, pero no las cachés de clima o mapa.');
...
t('privacy_pillar_network_body', 'En ligne, seules vos coordonnées GPS partent vers des services publics pour la météo, l’agronomie, les cartes, le cadastre, les zones protégées et le géocodage — jamais vos photos ni vos notes. Selon le pays : Open-Meteo, IGN / CNIG / basemap.de / OS / swisstopo / basemap.at / OpenTopo (BE, SE, JP, AU, DK, FI, IE, CA, NZ) / PDOK BRT (NL) / Kartverket (NO) / USGS (US), EEA, Natural England, OFEV, CadGIS, Nominatim (OpenStreetMap), et d’autres.', 'When online, only your GPS coordinates are sent to public services for weather, agronomy, maps, cadastre, protected areas and geocoding — never your photos or notes. By country: Open-Meteo, IGN / CNIG / basemap.de / OS / swisstopo / basemap.at / OpenTopo (BE, SE, JP, AU, DK, FI, IE, CA, NZ) / PDOK BRT (NL) / Kartverket (NO) / USGS (US), EEA, Natural England, FOEN, CadGIS, Nominatim (OpenStreetMap), and others.', 'Online werden nur Ihre GPS-Koordinaten an öffentliche Dienste für Wetter, Agronomie, Karten, Kataster, Schutzgebiete und Geokodierung gesendet — nie Ihre Fotos oder Notizen. Je nach Land: Open-Meteo, IGN / CNIG / basemap.de / OS / swisstopo / basemap.at / OpenTopo (BE, SE, JP, AU, DK, FI, IE, CA, NZ) / PDOK BRT (NL) / Kartverket (NO) / USGS (US), EEA, Natural England, BAFU, CadGIS, Nominatim (OpenStreetMap) und andere.', 'Online, solo le coordinate GPS vanno a servizi pubblici per meteo, agronomia, mappe, catasto, aree protette e geocodifica — mai le tue foto o note. Per paese: Open-Meteo, IGN / CNIG / basemap.de / OS / swisstopo / basemap.at / OpenTopo (BE, SE, JP, AU, DK, FI, IE, CA, NZ) / PDOK BRT (NL) / Kartverket (NO) / USGS (US), EEA, Natural England, UFAM, CadGIS, Nominatim (OpenStreetMap), e altri.', 'En línea, solo tus coordenadas GPS van a servicios públicos para clima, agronomía, mapas, catastro, zonas protegidas y geocodificación — nunca tus fotos ni notas. Según el país: Open-Meteo, IGN / CNIG / basemap.de / OS / swisstopo / basemap.at / OpenTopo (BE, SE, JP, AU, DK, FI, IE, CA, NZ) / PDOK BRT (NL) / Kartverket (NO) / USGS (US), EEA, Natural England, OFEV, CadGIS, Nominatim (OpenStreetMap), y otros.');
...
t('privacy_pillar_backup_body', 'Exportez régulièrement une sauvegarde (clé USB, cloud personnel). Sans mot de passe d’export, le fichier garantit seulement l’intégrité (sommes de contrôle), pas la confidentialité : quiconque a le fichier peut extraire les données. Avec un mot de passe, l’archive entière est chiffrée (PBKDF2 + AES-GCM). L’indice n’est qu’un aide-mémoire ; il ne permet pas de récupérer le mot de passe.', 'Export a backup regularly (USB drive, personal cloud). Without an export password, the file guarantees integrity only (checksums), not confidentiality: anyone who has the file can extract the data. With a password, the entire archive is encrypted (PBKDF2 + AES-GCM). The hint is a memory aid only; it cannot recover the password.', 'Exportieren Sie regelmäßig eine Sicherung (USB-Stick, persönliche Cloud). Ohne Exportpasswort gewährleistet die Datei nur die Integrität (Prüfsummen), nicht die Vertraulichkeit: Wer die Datei hat, kann die Daten extrahieren. Mit einem Passwort wird das gesamte Archiv verschlüsselt (PBKDF2 + AES-GCM). Der Hinweis ist nur eine Gedächtnisstütze; er kann das Passwort nicht wiederherstellen.', 'Esporta regolarmente un backup (chiavetta USB, cloud personale). Senza password di export, il file garantisce solo l’integrità (checksum), non la riservatezza: chiunque abbia il file può estrarre i dati. Con una password, l’intero archivio è cifrato (PBKDF2 + AES-GCM). Il promemoria è solo un aiuto memoria; non può recuperare la password.', 'Exporta copias con regularidad (USB, nube personal). Sin contraseña de exportación, el archivo garantiza solo la integridad (sumas de control), no la confidencialidad: quien tenga el archivo puede extraer los datos. Con contraseña, todo el archivo se cifra (PBKDF2 + AES-GCM). La pista es solo un recordatorio; no permite recuperar la contraseña.');
t('privacy_pillar_gps_title', 'GPS transparent', 'Transparent GPS', 'Transparentes GPS', 'GPS trasparente', 'GPS transparente');
t('privacy_pillar_gps_body', 'En premier plan, le GPS sert à repérer un arbre, afficher la carte et contextualiser la météo. Le suivi s’arrête lorsque l’application est en arrière-plan. Aucune position n’est envoyée à un serveur Yamadori.', 'In the foreground, GPS is used to scout a tree, display the map and contextualise weather. Tracking stops when the app is in the background. No location is sent to a Yamadori server.', 'Im Vordergrund dient GPS dem Sichten, der Kartenanzeige und der Wetterkontextualisierung. Die Verfolgung stoppt, wenn die App im Hintergrund ist. Es wird kein Standort an einen Yamadori-Server gesendet.', 'In primo piano, il GPS serve a rilevare un albero, mostrare la mappa e contestualizzare il meteo. Il tracciamento si interrompe quando l’app è in background. Nessuna posizione viene inviata a un server Yamadori.', 'En primer plano, el GPS sirve para registrar un árbol, mostrar el mapa y contextualizar el clima. El seguimiento se detiene cuando la app está en segundo plano. No se envía ninguna ubicación a un servidor Yamadori.');
...
t('privacy_pillar_honest_body', 'Comme un carnet papier dans votre sac, vos repérages restent lisibles sur l’appareil déverrouillé. Yamadori n’utilise pas la sauvegarde automatique Android vers Google. Nous recommandons un code de verrouillage et des exports protégés par mot de passe.', 'Like a paper notebook in your bag, your scouting entries remain readable on an unlocked device. Yamadori does not use Android’s automatic backup to Google. We recommend a screen lock and password-protected exports.', 'Wie ein Papiernotizbuch in der Tasche bleiben Ihre Sichtungen auf einem entsperrten Gerät lesbar. Yamadori nutzt nicht die automatische Android-Sicherung zu Google. Wir empfehlen eine Bildschirmsperre und passwortgeschützte Exporte.', 'Come un taccuino cartaceo in borsa, i tuoi rilevamenti restano leggibili su un dispositivo sbloccato. Yamadori non usa il backup automatico Android verso Google. Consigliamo un blocco schermo ed export protetti da password.', 'Como un cuaderno de papel en la mochila, tus registros siguen siendo legibles en un dispositivo desbloqueado. Yamadori no usa la copia de seguridad automática de Android hacia Google. Recomendamos bloqueo de pantalla y exportaciones protegidas por contraseña.');
...
t('onboarding_legal_title', 'Responsabilité & limites', 'Liability & limits', 'Verantwortung & Grenzen', 'Responsabilità e limiti', 'Responsabilidad y límites');
t('onboarding_legal_desc', "Le prélèvement d'arbres dans la nature (yamadori) nécessite l'autorisation du propriétaire ou du gestionnaire du terrain, et le respect des règles locales (espèces protégées, forêts, zonages). Les données affichées (cadastre, zones protégées, espèces, liens légaux) sont purement indicatives : elles ne constituent pas un avis juridique, une autorisation ni une garantie d'exactitude. L'éditeur de Yamadori ne saurait être tenu responsable d'une erreur, omission ou donnée obsolète. Vous restez seul responsable de vérifier la réglementation auprès des autorités compétentes avant tout prélèvement.", 'Collecting trees in the wild (yamadori) requires permission from the landowner or land manager, and compliance with local rules (protected species, forests, zoning). Data shown in this app (cadastre, protected areas, species, legal links) is for information only: it is not legal advice, collection permission or a guarantee of accuracy. The Yamadori publisher cannot be held liable for any error, omission or outdated data. You remain solely responsible for checking applicable regulations with the competent authorities before any collection.', 'Das Ausgraben von Bäumen in der Natur (Yamadori) erfordert die Erlaubnis des Grundeigentümers oder Flächenmanagers sowie die Einhaltung lokaler Regeln (geschützte Arten, Wald, Zonierung). Angezeigte Daten (Kataster, Schutzgebiete, Arten, Rechtslinks) sind nur indikativ: keine Rechtsberatung, keine Entnahmeerlaubnis und keine Genauigkeitsgarantie. Der Herausgeber von Yamadori haftet nicht für Fehler, Auslassungen oder veraltete Daten. Sie allein sind verantwortlich, die Vorschriften bei den zuständigen Behörden zu prüfen.', "Il prelievo di alberi in natura (yamadori) richiede l'autorizzazione del proprietario o gestore del terreno e il rispetto delle regole locali (specie protette, boschi, zonizzazioni). I dati mostrati (catasto, aree protette, specie, link legali) sono puramente indicativi: non sono consulenza legale, autorizzazione al prelievo né garanzia di accuratezza. L'editore di Yamadori non può essere ritenuto responsabile di errori, omissioni o dati obsoleti. Restate gli unici responsabili di verificare la normativa presso le autorità competenti prima di ogni prelievo.", 'La extracción de árboles en la naturaleza (yamadori) requiere permiso del propietario o gestor del terreno y el cumplimiento de las normas locales (especies protegidas, bosques, zonificación). Los datos mostrados (catastro, áreas protegidas, especies, enlaces legales) son meramente indicativos: no son asesoramiento jurídico, autorización de extracción ni garantía de exactitud. El editor de Yamadori no puede ser considerado responsable de errores, omisiones o datos obsoletos. Usted es el único responsable de verificar la normativa ante las autoridades competentes antes de cualquier extracción.');
...
t('settings_legal_disclaimer_link', "Clause d'information", 'Informational notice', 'Informationshinweis', 'Informativa', 'Aviso informativo');
...
const FALLBACK_LOCALES = new Set(['nl', 'sv', 'nb', 'pt', 'da', 'fi']);
