...
{#if treeStore.storageMigrationPending}
	{@const migrationPercent = Math.round(treeStore.storageMigrationProgress)}
	{@const migrationLabel = m.store_trees_migrating({ percent: migrationPercent })}
	<div
	...
	>
		<div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
			...
			<p class="mt-2 text-xs text-muted">{m.store_trees_migration_wait()}</p>
			<div
