...
{:else if loadFailed}
	<p class="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900" role="alert">
		{m.capture_climate_error()}
	</p>
{:else}
	<div
		class="flex min-h-[3rem] items-center justify-center gap-2 rounded-lg border border-gray-200 bg-white px-3"
		role="status"
		aria-busy="true"
